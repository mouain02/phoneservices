...
const Admin = () => {
  ...
  const submitBrand = async (e) => {
    ...
    const types = brandForm.types
      ...
      .filter(Boolean);
    const payload = {
      id: slugify(brandForm.id || brandForm.name),
      name: brandForm.name.trim(),
      types,
      logoUrl: brandForm.logoUrl || "",
    };
    if (!payload.id || !payload.name || types.length === 0) return;
  ...
  const submitModel = async (e) => {
    e.preventDefault();
    const payload = {
      id: slugify(modelForm.id || modelForm.name),
    ...
    if (!payload.id || !payload.brandId || !payload.name || !payload.type) return;
