    ...
    >
      <div className="container mx-auto flex items-center justify-between py-3 px-4">
        <button type="button" onClick={() => navigate("/")} className="flex items-center gap-3">
          <img src={logo} alt="Phoneservice HOUBA" className="h-10 w-10 object-contain" />
          <div>
            <span className="font-display text-xl font-900 tracking-tight text-foreground">
              Phoneservice
            </span>
            <span className="font-display text-xl font-900 tracking-tight text-primary"> HOUBA</span>
          </div>
        </button>
            ...
            {navLinks.map((link) => (
              link.external ? (
                <a
                  key={link.label}
                  href={link.href}
                  className="text-left font-medium py-2 text-sm text-foreground transition-colors hover:text-primary"
                >
                  {link.label}
                </a>
              ) : (
                <button
                  key={link.label}
                  onClick={() => handleNavClick(link.href)}
                  className={`text-left font-medium py-2 text-sm transition-colors hover:text-primary ${
                    link.label === "Get A Quote" ? "text-primary font-semibold" : "text-foreground"
                  }`}
                >
                  {link.label}
