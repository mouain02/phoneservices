...
const cleanMongoFields = (doc) => {
  ...
  const mapped = { ...rest };
  if (!mapped.id) mapped.id = String(_id);
  if (mapped.brandId && typeof mapped.brandId !== "string") {
    mapped.brandId = String(mapped.brandId);
  }
  return mapped;
};

const insertCatalogData = async ({ deviceTypes = [], brands = [], models = [] }) => {
  if (deviceTypes.length) {
    await DeviceType.insertMany(deviceTypes, { ordered: false });
  }

  const brandIdMap = new Map();
  for (const brand of brands) {
    const legacyId = brand.id || "";
    const { _id, id, ...brandDoc } = brand;
    const created = await Brand.create(brandDoc);
    if (legacyId) brandIdMap.set(String(legacyId), created._id);
  }

  const modelDocs = models
    .map((model) => {
      const { _id, id, brandId: rawBrandId, ...modelDoc } = model;
      let normalizedBrandId = rawBrandId;
      if (normalizedBrandId && !mongoose.Types.ObjectId.isValid(normalizedBrandId)) {
        normalizedBrandId = brandIdMap.get(String(normalizedBrandId));
      }
      if (!normalizedBrandId) return null;
      return { ...modelDoc, brandId: normalizedBrandId };
    })
    .filter(Boolean);

  if (modelDocs.length) {
    await Model.insertMany(modelDocs, { ordered: false });
  }
};
...
export const importCatalog = asyncHandler(async (req, res) => {
  ...
  await insertCatalogData({ deviceTypes, brands, models });
...
export const updateBrandById = asyncHandler(async (req, res) => {
  const updated = await Brand.findOneAndUpdate({ _id: req.params.id }, req.body, {
    new: true,
    runValidators: true,
  });
  if (!updated) {
...
export const deleteBrandById = asyncHandler(async (req, res) => {
  const target = await Brand.findOne({ _id: req.params.id });
  if (!target) {
    res.status(404);
    throw new Error("Brand not found");
  }
  ...
  await target.deleteOne();
  await Model.deleteMany({ brandId: target._id });
...
export const deleteModelById = asyncHandler(async (req, res) => {
  const target = await Model.findOne({ _id: req.params.id });
  if (!target) {
    res.status(404);
    throw new Error("Model not found");
  }
