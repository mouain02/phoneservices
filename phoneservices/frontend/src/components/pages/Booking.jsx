...
import { parsePrice } from "@/lib/utils/numbers";
...
const Booking = () => {
  ...
  return (
    <div className="min-h-screen bg-muted pb-8 pt-20 xs:pt-24">
      <div className="container mx-auto px-4">
        <div className="mx-auto max-w-6xl rounded-2xl bg-background p-4 shadow-card xs:rounded-3xl xs:p-6 md:p-10">
          <div className="mb-8 flex items-center justify-center gap-2 text-center xs:mb-10 xs:gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-primary text-primary xs:h-10 xs:w-10">
              <Check className="h-5 w-5" />
            </div>
            <div className="h-1 w-10 rounded-full bg-primary xs:w-20" />
            <div className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-primary text-primary xs:h-10 xs:w-10">
              <Check className="h-5 w-5" />
            </div>
            <div className="h-1 w-10 rounded-full bg-primary xs:w-20" />
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary font-bold text-primary-foreground xs:h-10 xs:w-10">3</div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <div>
              <button
                onClick={() => navigate(-1)}
                className="mb-5 inline-flex items-center gap-2 rounded-xl bg-primary/10 px-3 py-2 text-foreground xs:mb-6 xs:gap-3 xs:px-4"
              >
                <ArrowLeft className="h-4 w-4 text-primary" />
                <span className="text-xl font-light xs:text-2xl">Finalize <strong className="font-semibold">Booking</strong></span>
              </button>

              <h3 className="mb-4 text-xl font-semibold text-foreground xs:text-2xl">Select Service Method</h3>
              ...
              <div className="space-y-4">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <p className="text-lg font-semibold text-foreground xs:text-xl">Tampa Bay Residents Only <span className="text-sm text-primary">FREE</span></p>
                      <p className="text-muted-foreground">Service at your location</p>

                      <div className="mt-4">
                        <div className="mb-3 flex items-center gap-2">
                          <span className="h-2.5 w-2.5 rounded-full bg-primary" />
                          <p className="text-base text-foreground xs:text-xl">
                            Select <strong>date</strong>
                          </p>
                          <div className="h-px flex-1 bg-border" />
                        </div>

                        <div className="mb-4 flex items-center gap-1.5 xs:gap-2">
                          <button
                            type="button"
                          ...
                          <div className="grid flex-1 grid-cols-2 gap-2 xs:grid-cols-4 sm:grid-cols-8">
                            {dateOptions.map((opt) => (
                              <button
                                type="button"
                                key={opt.key}
                                ...
                                className={`rounded-xl border px-2 py-2 text-center transition-colors xs:rounded-2xl ${
                              ...
                              >
                                <p className="text-xs xs:text-sm">{opt.dayName}</p>
                                <p className="text-2xl font-bold leading-tight xs:text-3xl">{opt.dayNumber}</p>
                              </button>
                            ))}
                          </div>
                          <button
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-1">
                          {timeSlots.map((slot) => (
                            <button
                              ...
                              className={`min-w-fit rounded-xl border px-3 py-2 text-base font-semibold transition-colors xs:px-4 xs:text-lg ${
                                selectedTime === slot
                                  ? "border-primary text-foreground"
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-lg font-semibold text-foreground xs:text-xl">Ship My Device <span className="text-sm text-primary">FREE</span></p>
              ...
              <div className="mt-6 space-y-4 rounded-2xl border border-border p-4">
                <p className="text-base font-semibold text-foreground xs:text-lg">Your address</p>
            ...
            <div>
              <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-foreground xs:text-3xl md:text-4xl">{brandName} {modelName}</h3>
                  <p className="text-lg text-primary xs:text-xl">repair</p>
                </div>
              </div>

              <div className="rounded-2xl border border-border p-5">
                <div className="mb-3 flex items-start justify-between">
                  <p className="text-base text-muted-foreground xs:text-xl">{brandName} {modelName} {color}</p>
                  <p className="text-base xs:text-xl">{repairLabel}</p>
                </div>
                <div className="flex items-end justify-between border-t border-border pt-3">
                  <div>
                    <p className="text-3xl font-bold xs:text-4xl md:text-5xl">Total</p>
                    <p className="text-base text-muted-foreground xs:text-xl">incl. 0% Tax</p>
                  </div>
                  <p className="text-3xl font-black xs:text-4xl md:text-5xl">$ {amount.toFixed(2)}</p>
                </div>
              ...
              <div className="mt-8 space-y-5 rounded-2xl border border-border p-4">
                <div className="flex flex-col gap-3 xs:flex-row xs:gap-6">
                  <button
                    onClick={() => setCustomerType("private")}
                    className="inline-flex items-center gap-2 text-lg xs:text-xl"
                  >
                ...
                <p className="text-right text-base xs:text-lg">Total <strong>$ {amount.toFixed(2)}</strong> incl. 0% Tax</p>

                <button className="w-full rounded-2xl bg-primary px-6 py-3 text-2xl font-bold text-primary-foreground xs:py-4 xs:text-3xl">
                  Submit Request
                </button>
              </div>
            </div>
