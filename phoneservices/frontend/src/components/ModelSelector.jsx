...
import { slugifyLoose } from "@/lib/utils/slugify";
...
const buildImageCandidates = (model) => {
  const brand = slugifyLoose(model.brandId);
  const name = slugifyLoose(model.name);
  const id = slugifyLoose(model.id);
...
const ModelSelector = ({ brandId, deviceType, selectedModel, onSelect }) => {
  ...
  const models = (catalog.models || []).filter(
    (model) => model.brandId === brandId && model.type === deviceType
  );
  ...
  return (
    <div>
      ...
      <div className="grid max-h-[26rem] grid-cols-1 gap-3 overflow-y-auto pr-1 xs:max-h-[30rem] xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
        {models.map((model) => (
          <button
            key={model.id}
            onClick={() => onSelect(model)}
            className={`relative rounded-2xl border-2 p-3 text-left transition-all hover:border-primary hover:shadow-card xs:p-3.5 ${
