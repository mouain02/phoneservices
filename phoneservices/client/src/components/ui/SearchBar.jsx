...
import { searchModels, getBrands } from "@/components/data/phones";
...
const SearchBar = ({ onSelect, deviceType }) => {
  ...
  const results = useMemo(() => {
    ...
    const all = searchModels(query);
    return deviceType ? all.filter((m) => m.type === deviceType) : all;
  }, [query, deviceType]);

  const getBrandName = (brandId) =>
    getBrands().find((b) => b.id === brandId)?.name || brandId;
