...
const parseJsonOrThrow = async (res) => {
  ...
  try {
    ...
    const hint = text.trim().startsWith("<!DOCTYPE")
      ? "API returned HTML instead of JSON. Set REACT_APP_API_URL (or VITE_API_BASE_URL) to your live backend URL and redeploy."
    ...
    try {
      ...
      if (err instanceof TypeError) {
        ...
        const hint = isHttpsPage && isHttpApi
          ? "Mixed-content blocked: your site is https but API is http. Use an https API URL."
          : `Cannot reach API server at ${API_BASE}. Verify backend is live and CORS allows ${window.location.origin}.`;
        setError(hint);
      } else {
